...
namespace py3
{
    ...
    mxkit::Result CallMethod_t(Python2Api::Api& api, const char* file, const char* method, const char* paramFormat, ParamType param, std::function<void(PyObject*)> resultFunction)
    {
        Python2Api::GILUtils pyGILUtil(api);
        printf("Python27::ExcuteMethod file:%s, method:%s.\n", file, method);
        ...
        Py_DECREF(pyModule);
#endif
        printf("Python27::ExcuteMethod completed.\n");
        RETURN_RESULT(true);
    }

    mxkit::Result CallMethod(Python2Api::Api& api, const char* file, const char* method, const char* paramFormat, va_list* paramList, std::function<void(PyObject*)> resultFunction)
    {
        Python2Api::GILUtils pyGILUtil(api);
        printf("Python27::ExcuteMethod file:%s, method:%s.\n", file, method);
    ...
    // Clean up
        Py_DECREF(pyModule);
#endif
        printf("Python27::CallMethod completed.\n");
        RETURN_RESULT(true);
    }
...
mxkit::Result Python27::Initialize(const PythonApi::Info& info, const char* homeDir)
{
    if (!homeDir || 
    ...
    m_api = std::make_shared<Python2Api>();
    m_api->Init(info);
    ...
    if (ret == 0)
    {
        printf("Python27::Initialize fail:\n");
    ...
    printf("Python27::Initialize Completed !\n");
...
mxkit::Result Python27::Version(mx::PythonVersion* ver)
{
    if(!ver)
    RETURN_RESULT(false);
...
mxkit::Result Python27::Release()
{
    if (m_api->api.IsInitialized())//Py_IsInitialized())
    {
        Python2Api::GILUtils pyGILUtil(m_api->api);
        m_api->api.Finalize(); //Py_Finalize();
    }
...
mxkit::Result Python27::ExcuteFile(const char* file)
{
    Python2Api::GILUtils pyGILUtil(m_api->api);
    printf("Python27::ExcuteFile:%s.\n", file);

    if (!m_api->api.IsInitialized())//Py_IsInitialized())
        RETURN_RESULT(false);

    PyObject* PyFileObject = m_api->api.File_FromString((char*)file, "r");
    FILE* pyFile = m_api->api.File_AsFile(PyFileObject);
    //FILE* pyFile = m_api->api.fopen(file, "r+");// ::_Py_fopen(file, "r+");
    if (!pyFile)
        RETURN_RESULT(false);

    int res = m_api->api.Run_SimpleFileExFlags(pyFile, file, true, NULL);
    ...
    printf("Python27::ExcuteFile completed: %d.\n", res);
    RETURN_RESULT(true);
}

mxkit::Result Python27::ExcuteMethod(const char* file, const char* method, int param, int* result)
{
...
mxkit::Result Python27::ExcuteMethod(const char* file, const char* method, int* result, const char* paramFormat, ...)
{
...
mxkit::Result Python27::ExcuteMethod(const char* file, const char* method, float param, float* result)
{
...
mxkit::Result Python27::ExcuteMethod(const char* file, const char* method, float* result, const char* paramFormat, ...)
{
    va_list paramList = nullptr;
    va_start(paramList, paramFormat);
...
mxkit::Result Python27::ExcuteMethod(const char* file, const char* method, const char* param, char** result)
{
...
mxkit::Result Python27::ExcuteMethod(const char* file, const char* method, char** result, const char* paramFormat, ...)
{
    va_list paramList = nullptr;
    va_start(paramList, paramFormat);
