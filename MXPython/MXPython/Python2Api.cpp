...
Python2Api::~Python2Api()
{
    PythonApi::Free(&m_info);
}

bool Python2Api::Init(const PythonApi::Info& info)
{
    if (!info.dllModule)
        return false;
    ...
    _GET_PROCESS_ADDRESS_(mxModule, PyFile_FromString, api.File_FromString);
    _GET_PROCESS_ADDRESS_(mxModule, PyFile_AsFile, api.File_AsFile);
...
Python2Api::GILUtils::GILUtils(Api& api) : m_api(api)
{
